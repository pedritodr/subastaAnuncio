    ...
    public function index_post()
    {
        ...
        if ($auth) {
            if ($auth->is_active == 1) {
                ...
                $this->response(['user_object' => $auth, 'status' => 200, 'security_token' => $token, 'user_id' => $auth->user_id, 'email' => $email, 'role_id' => $auth->role_id, 'name' => $auth->name, 'cedula' => $auth->cedula, 'phone' => $auth->phone]);
            } else {
