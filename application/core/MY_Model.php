...
class MY_Model extends CI_Model
{
    ...
    public function __construct()
    {
        ...
        $i = $this->session->userdata('idioma');
        if ($i == 'es') {
            $this->idioma = "Español";
        } else {
            $this->idioma = "Ingles";
    ...
    public function relate($row)
    {
        ...
        foreach ($this->belongs_to as $key => $value) {
            if (is_string($value)) {
                $relationship = $value;
                $options = array('primary_key' => $value . 'id', 'model' => $value . '_model');
            } else {
                $relationship = $key;
                $options = $value;
            }
    ...
    public function fetch_fields($limit, $start, $order = "asc")
    {
    ...
    public function todo_activo()
    {
        $query = "SELECT * FROM " . $this->_table . " WHERE estado <> '0' ORDER BY id ASC";
        return $this->db->query($query)->result();
    }

    public function get_last_id($table = null)
    {
        $query = "SELECT MAX(id) AS last_id FROM " . $this->_table . "";
