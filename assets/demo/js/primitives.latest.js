...
primitives.orgdiagram.BaseController = function () {
    ...
    /* primitives.orgdiagram.ItemConfig or primitives.famdiagram.ItemConfig */

    // Organizational chart definition 
    this._orgItems = {};
    /* primitives.orgdiagram.OrgItem */
    this._orgItemChildren = {};
    /* key: primitives.orgdiagram.OrgItem.id value: array of primitives.orgdiagram.OrgItem.id having  OrgItem.parent equal to key */
    this._orgItemRoot = null; // primitives.orgdiagram.OrgItem

    this._orgPartners = {};
    /* key: primitives.orgdiagram.OrgItem.id value: array of primitives.orgdiagram.OrgItem.id */

    // Visual tree definition
    this._treeItems = {};
    /* key: primitives.orgdiagram.TreeItem.id value:primitives.orgdiagram.TreeItem */
    this._treeItemCounter = 0;
    /* counter of primitives.orgdiagram.TreeItem.id*/
    this._treeItemsByUserId = {};
    /* key: ItemConfig.id value:primitives.orgdiagram.TreeItem */

    this._treeLevels = [];
    /* array of primitives.orgdiagram.TreeLevel */
    this._leftMargins = {};
    this._rightMargins = {};
...
primitives.orgdiagram.BaseController.prototype._virtualBind = function () {
    ...
    if ('ontouchstart' in document.documentElement) {//ignore jslint
        this.m_scrollPanel[0].addEventListener("gesturestart", self.onGestureStartHandler = function (event) {
            self.onGestureStart(event);
        }, false);
        this.m_scrollPanel[0].addEventListener("gesturechange", self.onGestureChangeHandler = function (event) {
            self.onGestureChange(event);
        }, false);
    }

    this.options.onDefaultTemplateRender = function (event, data) {
        self._onDefaultTemplateRender(event, data);
    };
    this.options.onCheckBoxTemplateRender = function (event, data) {
        self._onCheckBoxTemplateRender(event, data);
    };
    this.options.onGroupTitleTemplateRender = function (event, data) {
        self._onGroupTitleTemplateRender(event, data);
    };
    this.options.onButtonsTemplateRender = function (event, data) {
        self._onButtonsTemplateRender(event, data);
    };
    this.options.onAnnotationLabelTemplateRender = function (event, data) {
        self._onAnnotationLabelTemplateRender(event, data);
    };
};
    ...
    for (index = 0, len = this._treeLevels.length; index < len; index += 1) {
        ...
        for (index2 = 0, len2 = treeLevel.treeItems.length; index2 < len2; index2 += 1) {
            ...
            for (index3 = 0, len3 = toItems.length; index3 < len3; index3 += 1) {
                ...
                result[treeItem.id][itemTo] = (len3 == 1) ? {
                    id: treeItem.id,
                    weight: treeItemToWeight
                } : {id: treeItem.id, partnerid: itemTo, weight: treeItemToWeight};

                if (!result.hasOwnProperty(itemTo)) {
                    result[itemTo] = {};
                }
                result[itemTo][treeItem.id] = (len3 == 1) ? {
                    id: treeItem.id,
                    weight: treeItemWeight
                } : {id: treeItem.id, partnerid: itemTo, weight: treeItemWeight};
            }
        }
    }
    return result;
...
primitives.orgdiagram.BaseController.prototype._redrawConnector = function (hasGraphics, buffer, paletteManager, parentTreeItem, parentTreeLevel) {
    ...
    if (parentTreeItem.partners.length > 1 || parentTreeItem.extraPartners.length > 0) {
        ...
        if (partnerPoints.length > 0) {
            partnerPoints.sort(function (a, b) {
                return a.x - b.x;
            });
...
primitives.orgdiagram.BaseController.prototype._stretchToAvoidPageMargins = function (diagramSize, panelSize) {
    ...
    sortedItems = primitives.common.mergeSort(treeLevelsItems, function (item) {
        return self._treeItems[item].offset;
    });
...
primitives.orgdiagram.BaseController.prototype._getNewTreeItem = function (options) {
    ...
    /* TreeItem reference by visual Id */

    orgItem = options.orgItem;
    if (orgItem != null) {
        this._treeItemsByUserId[orgItem.id] = result;
        /* TreeItem reference by user Id ItemConfig.id */
...
primitives.common.getReferencesGraph = function (references) {
    var result = {},
        targets = primitives.common.invertReferences(references, function (item) {
            return item.targets;
        }),
...
primitives.famdiagram.Controller.prototype._virtualBind = function () {
    ...
    this.options.onLabelAnnotationTemplateRender = function (event, data) {
        self._onDefaulLabelAnnotationTemplateRender(event, data);
    };
};
...
primitives.famdiagram.Controller.prototype._virtualCreateOrgTree = function () {
    ...
    this.showInvisibleSubTrees = true;
    /* in regular org diagram we hide branch if it contains only invisible nodes, 
     in the family chart we use invisible items to draw connectors across multiple levels */

    this.showElbowDots = true;
    ...
    // User ItemConfigs
    this._treeItemConfigs = {};
    /* Hash where key: primitives.famdiagram.ItemConfig.id and value: ItemConfig*/

    // Family Chart definition - this is source of chart
    this._famItems = {};
    /* Hash where key: FamilyItem.id and value: FamilyItem */
    ...
    this._orgPartners = {};
    /* this is items which share the same children with this item */
    ...
    if (this.options.items.length > 0) {
        ...
        for (key in this._famItems) {
            if (this._famItems.hasOwnProperty(key)) {
                ...
                famItem.logicalChildren.sort(function (a, b) {
                    return self._famItems[a].level - self._famItems[b].level;
                });//ignore jslint
                famItem.logicalParents.sort(function (a, b) {
                    return self._famItems[b].level - self._famItems[a].level;
                });//ignore jslint
            }
        }
...
primitives.famdiagram.Controller.prototype._getGrandChildren = function () {
    var itemId, orgItem,
        ...
        result = {};
    /* Key = primitives.orgdiagram.OrgItem.id, Value= Hash {} having Key = level and Value = number of grand children*/
...
primitives.famdiagram.Controller.prototype._balanceItems = function (sequence, leftId, rightId, graphGravities, grandChildren) {
    ...
    slots.add(new primitives.famdiagram.Slot(null));
    /* first empty slot */
...
primitives.orgdiagram.Controller.prototype._defineLogicalParent = function (logicalParentItem, treeItem) {
    ...
    for (parentKey in logicalParents) {
        if (logicalParents.hasOwnProperty(parentKey)) {
            ...
            if (this._debug && primitives.common.indexOf(parent.logicalChildren, treeItem, function (a, b) {
                    return a.id == b.id;
                }) > 0) { //ignore jslint
...
primitives.orgdiagram.TreeItem = function (id) {
    ...
    this.extraPartners = [];
    /* this is partners which are not part of organizational chart tree, but must be connected */
...
primitives.orgdiagram.TreeLevel = function (level) {
    ...
    this.partnerConnectorOffset = 0;
    /* number of overlapping horiontal connection lines between partners in level */
