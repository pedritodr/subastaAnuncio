...
primitives.common.Callout.prototype._drawSegment = function (a, b, c, d, e, f, g, h) {
    var i = this._offsetPoint(b, c, f), j = this._offsetPoint(c, b, f), d = this._offsetPoint(c, d, f), e = this.m_graphics.getPxSize(e, b.distanceTo(c) / 2);
...
primitives.common.Connector.prototype.draw = function (a, b, c) {
    var d, e, f, g, h, i, j, k, l, m = 6 * this.lineWidth, n, p, q = 0, r = 0, t, v = new primitives.common.PaletteItem({
        lineColor: this.color,
        lineWidth: this.lineWidth,
        lineType: this.lineType
    }), s = new primitives.common.PolylinesBuffer, w = s.getPolyline(v), a = (new primitives.common.Rect(a)).offset(this.offset), b = (new primitives.common.Rect(b)).offset(this.offset);
    i = [];
    ...
    if (a.right() + q < b.left() || a.left() > b.right() + q)if (a.left() > b.right() ? (d = new primitives.common.Point(a.left(), a.verticalCenter()), e = new primitives.common.Point(b.right(), b.verticalCenter()), q = 7, r = 3) : (d = new primitives.common.Point(a.right(), a.verticalCenter()), e = new primitives.common.Point(b.left(), b.verticalCenter()), q = 3, r = 7), f = new primitives.common.Rect(d, e), l = d.y <= e.y, k = d.x <
...
primitives.common.Graphics.prototype.dot = function (a, b, c, d, e, f) {
    var g = this.m_activePlaceholder, h = this.m_cache.get(g.name, g.activeLayer.name, "dot"), i = void 0 !== f.lineWidth && void 0 !== f.borderColor, a = {
        position: "absolute",
        width: c - (i ? 1 : 0),
        top: Math.round(b),
        left: Math.round(a),
...
primitives.common.Graphics.prototype.template = function (a, b, c, d, e, f, g, h, i, j, k, l, m) {
    var c = this.m_activePlaceholder, d = "template" + (null !== j ? j : primitives.common.hashCode(i)), n = 0, j = this.m_cache.get(c.name, c.activeLayer.name, d);
...
primitives.common.VmlGraphics.prototype.polyline = function (a, b) {
    var c = this.m_activePlaceholder, d = new primitives.common.Rect(c.rect), e = new primitives.common.Rect(0, 0, 10 * d.width, 10 * d.height), f = new primitives.common.Element(this.prefix + ":shape"), g, h, i, j, k;
...
primitives.orgdiagram.BaseController.prototype._getNextLevelTreeItem = function (a, b) {
    var c = a.actualPosition.horizontalCenter(), d, e, f, g, h = null, i = null, j, k, l, m = b ? this._treeLevels.slice(a.level + 1, this._treeLevels.length) : this._treeLevels.slice(0, a.level).reverse();
    f = 0;
    for (g = m.length; f < g; f += 1) {
        k = m[f].treeItems;
        d = 0;
        for (e = k.length; d < e; d += 1)if (l = this._treeItems[k[d]], 1 == l.actualVisibility)if (j = Math.abs(c - l.actualPosition.horizontalCenter()), j < i || null == i)i = j, h = l; else break;
        if (null != i)break
    }
    return h
};
primitives.orgdiagram.BaseController.prototype._onKeyDown = function (a) {
    var b, c, d, e = null != this._highlightTreeItem ? this._highlightTreeItem : this._cursorTreeItem, f, g, h = null, i;
    if (null != e) {
        switch (a.which) {
            case 13:
...
primitives.orgdiagram.BaseController.prototype._drawCursor = function () {
    ...
    null !== a && 1 == a.actualVisibility && (this.graphics.activate("placeholder", 6), b = a.actualPosition, c = new primitives.common.Rect(a.contentPosition), d = a.template.cursorPadding, c.offset(d.left, d.top, d.right, d.bottom), e = new primitives.common.RenderEventArgs, e.context = a.itemConfig, e.isCursor = a.isCursor, e.isSelected = a.isSelected, e.templateName = a.template.name, this.transform.transformRect(b.x, b.y, b.width, b.height,
        !0, this, function (b, d, h, i) {
            this.graphics.template(b, d, h, i, c.x, c.y, c.width, c.height, a.template.cursorTemplate, a.template.cursorTemplateHashCode, a.template.cursorTemplateRenderName, e, {"border-width": a.template.cursorBorderWidth})
        }))
};
...
primitives.orgdiagram.BaseController.prototype._createLabel = function (a, b, c, d, e) {
    var f, g, h = null, i = this.options.labelOffset, j;
    j = e.orgItem;
    ...
    for (c = this._treeLevels.length; b < c; b += 1) {
        ...
        for (e = a.treeItems.length; d < e; d += 1) {
            ...
            for (g = k.length; f < g; f += 1)l = k[f], i = this._treeItems[l], i = i.actualIsActive ? 100 : 1, m.hasOwnProperty(h.id) || (m[h.id] = {}), m[h.id][l] = 1 == g ? {
                ...
                partnerid: l,
                weight: i
            }, m.hasOwnProperty(l) || (m[l] = {}), m[l][h.id] = 1 == g ? {id: h.id, weight: j} : {
                id: h.id,
                partnerid: l,
                weight: j
            }
...
primitives.orgdiagram.BaseController.prototype._getBackgroundManager = function () {
    var a = new primitives.common.perimeter.Manager, b, c, d, e, f, g, h, i, j, k, l;
    if (0 < this._treeLevels.length) {
        i = [];
        f = [];
        b = 0;
        for (c = this._treeLevels.length; b < c; b += 1)if (h = j = null, d = this._treeLevels[b], 4 != d.actualVisibility) {
            i.push(b);
            g = [];
            k = d.treeItems;
            d = 0;
...
primitives.orgdiagram.BaseController.prototype._addTreeLevelPerimeters = function (a, b, c, d, e, f, g) {
    var h = Math.floor(this.options.normalItemsInterval / 2), e = null != e ? Math.floor(3 * (e.shift + e.depth) / 7 + 4 * f.shift / 7) : f.shift - h, g = null != g ? Math.floor(3 * (f.shift + f.depth) / 7 + 4 * g.shift / 7) : f.shift + f.depth + h, i = h = null, j = f.treeItems, k, l;
    k = 0;
...
primitives.orgdiagram.BaseController.prototype._addPerimeter = function (a, b, c, d, e, f, g, h, i, j) {
    var k = [], l = primitives.common.perimeter.SegmentItem, m = Math.floor(this.options.normalItemsInterval / 2), h = null != h ? Math.floor(3 * (h.offset + h.actualSize.width) / 7 + 4 * i.offset / 7) : i.offset - m, j = null != j ? Math.floor(3 * (i.offset + i.actualSize.width) / 7 + 4 * j.offset / 7) : i.offset + i.actualSize.width + m;
...
primitives.orgdiagram.BaseController.prototype._redrawConnectors = function () {
    var a = this.graphics.activate("placeholder", 2), b, c, d, e, f, g, h, i;
...
primitives.orgdiagram.BaseController.prototype._redrawConnector = function (a, b, c, d, e) {
    var f = this, g, h, i, j, k, l, m, n, p, q, r, t, v = 0, s, w = 0 == this.options.connectorType, u;
    ...
    if (0 < d.visualChildren.length) {
        ...
        !g && 0 < h.length && (this.transform.transformPoints(r, t, r, e.shift + e.connectorShift, !0, this, function (a, e, g, j) {
            var k = Math.round(f.options.elbowDotSize / 2), l = c.getPalette(v), m = b.getPolyline(l), m = b.getPolyline(m.arrowPaletteItem);
...
primitives.orgdiagram.BaseController.prototype._drawTopConnectors = function (a, b, c, d, e, f, g, h) {
    var i = this, j, k, l, m = [], n, p, q;
    if (f) {
        m = [];
        f = 0;
        for (k = e.length; f < k; f += 1)if (l = e[f], l.x < c && !l.isSquared)m.push(l); else break;
        k = m.length;
        0 < k && this._drawAngularConnectors(a, b, c, d, m, !1, h);
        m = [];
        for (j = e.length - 1; j >= f; j -= 1)if (l = e[j], l.x > c && !l.isSquared)m.push(l); else break;
        k = m.length;
        0 < k && this._drawAngularConnectors(a, b, c, d, m, !1, h);
        m = {};
        k = {};
...
primitives.orgdiagram.BaseController.prototype._drawAngularConnectors = function (a, b, c, d, e, f, g) {
    var h = null, i = e.length, j, k, l, m, h = f ? c : e[i - 1].x;
...
primitives.orgdiagram.BaseController.prototype._setOffset = function (a) {
    var b = this._treeLevels[a.level], c = this._itemsInterval[0 === a.visibility ? b.currentvisibility : a.visibility] / 2, d, e, f, g, h, i;
    ...
    if (0 < a.visualChildren.length) {
        ...
        if (0 < c)this._offsetItemChildren(a, c); else if (0 > c && (c = -c, this._offsetItem(a, c), d = null, e = {}, g = null, h = this._getParentItem(a), null !== h)) {
            for (b = h.visualChildren.length - 1; 0 <= b; b -= 1)if (i = h.visualChildren[b], i === a)d = []; else if (null !== d)if (f = this._getGapBetweenSiblings(i, a), e[i.id] = f, 0 < f)d.splice(0, 0, i); else {
                g =
                    i;
                break
            }
...
primitives.orgdiagram.BaseController.prototype._createVisualTreeItem = function (a) {
    ...
    if (null != f) {
        ...
        for (e = f.length; h < e; h += 1)if (c = f[h], this.showInvisibleSubTrees || this._hasVisibleChildren(c)) {
            ...
            switch (b.actualItemType) {
                ...
                case 6:
                    d = this._treeItems[a.visualParentId];
                    if (a.connectorPlacement & 2)d.visualChildren.splice(i, 0, b), b.connectorPlacement = 6, b.gravity = 2; else if (a.connectorPlacement & 8)d.visualChildren.splice(d.visualChildren.length - j, 0, b), b.connectorPlacement = 12, b.gravity = 1;
                    else {
                        switch (c.adviserPlacementType) {
                            case 2:
    ...
    switch (a.actualItemType) {
        ...
        default:
            if (null !== a.visualParentId) {
                d = this._treeItems[a.visualParentId];
                for (h = i; h < d.visualChildren.length - j; h += 1)if (e = d.visualChildren[h], e.id == a.id)b.push(e); else switch (e.actualItemType) {
                    case 7:
                    case 8:
                    case 6:
                        b.push(e)
    ...
    if (null !== a.visualAggregatorId && (e = this._treeItems[a.visualAggregatorId], 0 < e.visualChildren.length && (j = this._getAssitantsDepth(a), 1 < j)))for (h = 0; h < j - 1; h += 1)e = this._createNewVisualAggregator(e);
    if (0 < a.visualChildren.length &&
    ...
    for (e = f.length; h < e; h += 1)if (c = f[h], 1 < c) {
        g = 0;
        for (i = d[h].length; g < i; g += 1)if (k = d[h][g], 0 < k.visualChildren.length)for (j = c; 1 < j;)k = this._createNewVisualAggregator(k), j -= 1
    }
    this._layoutPartners(a, b)
};
primitives.orgdiagram.BaseController.prototype._layoutPartners = function (a, b) {
    var c, d, e, f, g, h = 0, i = [], j;
    f = [];
    if (1 < b.length) {
...
primitives.orgdiagram.BaseController.prototype._getAllLogicalParents = function (a, b) {
    var c = [], d = [], e, f = [], g, h, i, j, k = {}, b = b || "logicalParents";
    i = 0;
    for (j = a.length; i < j; i += 1)if (e = a[i], !k.hasOwnProperty(e.id)) {
        k[e.id] = !0;
        c.push(e);
...
primitives.common.perimeter.LinkedHashItems.prototype.insertAfter = function (a, b, c) {
    ...
    if (null == a)this.unshift(b, c); else {
...
primitives.common.perimeter.LinkedHashItems.prototype.remove = function (a) {
    var b = this.prevKeys[a], c = this.nextKeys[a];
...
primitives.common.perimeter.LinkedHashItems.prototype._iterate = function (a, b, c, d) {
    ...
    if (null != b)for (; null != c;)e = this.segmentsHash[c], null != e && b(e), c = c == d ? null : a ? this.nextKeys[c] : this.prevKeys[c]
};
...
primitives.common.perimeter.Manager.prototype.getMergedPerimeters = function (a) {
    ...
    d = 0;
    for (e = a.length; d < e; d += 1)if (f = a[d], !c[f]) {
        c[f] = !0;
...
primitives.common.getGraphSpanningTree = function (a, b, c) {
    ...
    for (h[b] = null; 0 < i;) {
        ...
        for (f in e)if (e.hasOwnProperty(f)) {
            m = a[f];
            b = !1;
            for (n in m)if (m.hasOwnProperty(n) && !h.hasOwnProperty(n) && (p = m[n], b = !0, !k || 0 <= c(p, k)))j = n, k = p, l = f;
            b || g.push(f)
        }
        if (null == j)break; else e[j] = !0, i += 1, h[j] = l, d.hasOwnProperty(l) || (d[l] = []), d[l].push(j);
        b = 0;
...
primitives.common.optimizeReferences = function (a, b) {
    var c = {}, d = [], e = {}, f = {}, g, h, i, j, k, l, m, n, p, q, r;
    ...
    for (; !primitives.common.isEmptyObject(f);) {
        ...
        for (g in f)if (f.hasOwnProperty(g) && (j = f[g], d.push(j), !j.processed))for (m in l = primitives.common.getGraphSpanningTree(i, g, function (a,
                                                                                                                                                        b) {
...
primitives.common.getGraphGrowthSequence = function (a, b, c) {
    ...
    for (h[b] = null; 0 < i;) {
        ...
        for (f in e)if (e.hasOwnProperty(f)) {
            m = a[f];
            b = !1;
            for (n in m)if (m.hasOwnProperty(n) && !h.hasOwnProperty(n) && (p = c(m[n]), b = !0, null == l[n] && (l[n] = 0), l[n] += p, !k || l[n] > k))j = n, k = l[n];
            b || g.push(f)
        }
        if (null == j)break; else e[j] = !0, i += 1, h[j] = !0, d.push(j);
        b = 0;
        for (j = g.length; b < j; b += 1)delete e[g[b]], i -= 1
    }
    return d
...
primitives.common.mergeSort = function (a, b, c) {
    var d = null, e, f, g, h, i, j, k, l, m, n, p, q;
    switch (a.length) {
        case 0:
            d = [];
            break;
...
primitives.common.stackSegments = function (a, b) {
    var c = 0, d, e, f, g, h, i, j, k = [], l, m, n, p, q, r, t = {};
    a.sort(function (a, b) {
...
primitives.famdiagram.Controller.prototype._virtualCreateOrgTree = function () {
    var a, b, c, d, e, f, g, h = {}, i, j, k, l = [], m, n, p, q, r = this, t, v, s, w, u, x;
    ...
    if (0 < this.options.items.length) {
        ...
        p = {};
        v = [];
        for (m in this._famItems)if (this._famItems.hasOwnProperty(m)) {
            d = this._famItems[m];
            q = {};
        ...
        for (b = v.length; a < b; a += 1) {
            ...
            u = !1;
            e = 0;
            for (f = s.logicalChildren.length; e < f; e += 1)if (c = this._famItems[s.logicalChildren[e]], c.logicalParents.length == w.length) {
                x = !0;
                d = 0;
                for (g = c.logicalParents.length; d < g; d += 1)if (!q.hasOwnProperty(c.logicalParents[d])) {
                    x = !1;
                    break
                }
                if (x) {
        ...
        if (0 < this.options.annotations.length) {
            ...
            for (i in c)if (c.hasOwnProperty(i)) {
                ...
                for (a = 0; a < b.length; a += 1)t = b[a], this._addLabelAnnotation(l, t.fromItem, t.toItems, function () {
                    ...
                    a = new primitives.famdiagram.FamilyItem({
                        ...
                        isVisible: !0, isActive: !1, itemConfig: t, levelGravity: 1
                    });
                    r._famItems[a.id] = a;
                    return a.id
            ...
            for (m in this._famItems)if (this._famItems.hasOwnProperty(m)) {
                d = this._famItems[m];
                i = l[d.id];
                if (null != i)for (j in i)i.hasOwnProperty(j) && (e = d, i[j] != d.id && (e = this._famItems[i[j]]),
                ...
                f = p[d.id];
                if (null != f)for (k in f)f.hasOwnProperty(k) && (e = this._famItems[k], c = d, f[k] == d.id && !l.hasOwnProperty(k) && (e.logicalChildren.push(c.id), c.logicalParents.push(e.id), e.children.push(c.id), c.parents.push(e.id)))
            }
        }
        a = {};
        ...
        this._eliminateManyToMany(this._famItems, function () {
            ...
            return new primitives.famdiagram.FamilyItem({
                ...
                isVisible: !1,
                isActive: !1,
                itemConfig: new primitives.famdiagram.ItemConfig({id: r.maximumFamItemId}),
                levelGravity: 2
            })
        });
        ...
        if (this._debug)for (n in this._famItems)if (this._famItems.hasOwnProperty(n)) {
            d = this._famItems[n];
            i = d.logicalChildren;
        ...
        if (0 < this._families.length) {
            ...
            for (; l.length < this._families.length;) {
                a = 0;
                for (b = this._families.length; a < b; a += 1)if (n = this._families[a], !m.hasOwnProperty(n.id))if (j = primitives.common.getGraphSpanningTree(h, n.id, function (a, b) {
                        return a - b
        ...
        for (b = l.length; a < b; a += 1) {
            ...
            for (c = 0; c < h.length; c += 1)if (i = h[c], p = this._orgItems[i.toItem], i = this._orgItems[i.fromItem], !0 == m[p.familyId]) {
...
primitives.famdiagram.Controller.prototype._balanceChildrenForItem = function (a, b, c) {
    var d, e, f, g, h, i, j, k, l, m, n, p = [c], q, r = [], t, v, s, w, u;
    for (c.childIndex = 0; 0 < p.length;) {
        c = 0;
        for (j = p.length; c < j; c += 1)if (q = p[c], d = {}, e = {}, this._orgItemChildren.hasOwnProperty(q.id)) {
...
primitives.famdiagram.Controller.prototype._balanceItems = function (a, b, c, d, e) {
    var f = [], g, h, i, j, k, l, m, n, p, q, r, t, v, s, w, u, x, z, A, y;
    ...
    for (g = 0; g < a.length; g += 1)if (j = a[g], j != b && j != c)for (x in p = n = m = l = k = null, h.loop(function (a) {
        var b, c, f, g;
        if (null == a.itemId) {
            s = e[j];
            v = t = r = q = 0;
            ...
            for (b in d[j])if (d[j].hasOwnProperty(b))for (c in f = d[j][b], f)f.hasOwnProperty(c) && (g = h.getSlot(c), null != g && (g.position < a.position ? (q += (a.left[b] || 0) - (g.left[b] || 0), t += Math.abs(g.balance + 1)) : (q += (a.right[b] || 0) - (g.right[b] || 0), t += Math.abs(g.balance - 1)), r += Math.abs(g.position - a.position)));
    ...
    }, u), d[j])if (d[j].hasOwnProperty(x))for (A in z = d[j][x], z)z.hasOwnProperty(A) && (y = h.getSlot(A), null != y && (0 > y.position ? (y.balance += 1, u.balance -= 1, h.backwardLoop(function (a) {
...
primitives.famdiagram.Controller.prototype._findLargestRoot = function () {
    var a = null, b, c, d = {}, e, f, g, h, i, j, k = {}, l, m, n;
    ...
    for (h = this.maximumLevel; h >= this.minimumLevel; h -=
        ...
        for (g = e.length; f < g; f += 1) {
            ...
            k[b.id][b.id] = !0;
            i = 0;
            for (j = b.logicalParents.length; i < j; i += 1)if (n = b.logicalParents[i], this._famItemsNonExtracted.hasOwnProperty(n) && (k.hasOwnProperty(n) || (k[n] = {}), k[n][b.id] = !0, k.hasOwnProperty(b.id)))for (c in l = k[b.id], l)l.hasOwnProperty(c) && (k[n][c] = !0)
        }
    }
    b = 0;
...
primitives.famdiagram.Controller.prototype._eliminateManyToMany = function (a, b) {
    ...
    for (e in a)if (a.hasOwnProperty(e) && (j = a[e], 1 < j.logicalChildren.length)) {
        k = [];
        f = 0;
...
primitives.orgdiagram.Controller.prototype._hideRootConnectors = function (a) {
    for (var b, c, d, e, f, g, h, i, j; 0 < a.length;) {
        ...
        for (d = a.length; c < d; c += 1)if (g = a[c], !g.isVisible && this._orgItemChildren.hasOwnProperty(g.id)) {
            ...
            j = !0;
            e = 0;
            for (f = h.length; e < f; e += 1)if (i = h[e], 0 != i.itemType) {
                j = !1;
                break
            }
            if (j) {
